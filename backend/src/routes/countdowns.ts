...
router.post('/', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const { title, target_date, icon, color, is_pinned } = req.body;
    if (!title || !target_date) return res.status(400).json({ error: '标题和目标日期为必填项' });
    const db = getDB();
    const id = generateId();
    db.prepare('INSERT INTO countdowns (id, user_id, title, target_date, icon, color, is_pinned) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, req.user!.id, title, target_date, icon || null, color || null, is_pinned ? 1 : 0);
...
router.put('/:id', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!countdown) return res.status(404).json({ error: '倒数日不存在' });
    const { title, target_date, icon, color, is_pinned } = req.body;
    db.prepare('UPDATE countdowns SET title = COALESCE(?, title), target_date = COALESCE(?, target_date), icon = COALESCE(?, icon), color = COALESCE(?, color), is_pinned = COALESCE(?, is_pinned) WHERE id = ?').run(title, target_date, icon, color, is_pinned !== undefined ? (is_pinned ? 1 : 0) : null, req.params.id);
