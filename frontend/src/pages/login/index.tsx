...
export default function LoginPage() {
  ...
  const onFinish = async (values: { username: string; password: string; confirmPassword?: string }) => {
    ...
    try {
      ...
      Toast.show({ content: err.response?.data?.message || '操作失败', position: 'center' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="login-page min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-purple-500 to-indigo-600 px-6">
      <div className="login-logo mb-8 animate-bounce">
      ...
      <div className="login-card w-full max-w-sm bg-white rounded-2xl p-6 shadow-xl animate-fade-in">
        ...
        <div className="text-center mt-4">
          ...
          <span className="text-purple-600 text-sm font-medium cursor-pointer ml-1 transition-colors hover:text-purple-800"
